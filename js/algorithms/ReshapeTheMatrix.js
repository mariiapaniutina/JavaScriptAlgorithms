...
var matrixReshape = function(nums, r, c) {
    ...
    //checking if its possible to re-shape
    if (rows*cols !== r*c) {
        return nums;
    }
    
    //build list with all numbers from given matrix
    var matrixList = [];
    for (var i=0; i<rows; i++){
        for (var j=0; j<cols; j++){
            matrixList.push(nums[i][j]);
        }
    }
        
    //building new (re-shaped) matrix
    var matrix = [];
    for (var i=0; i<r; i++){
        matrix.push([]);
        for (var j=0; j<c; j++){
            var el = matrixList.shift();
            matrix[i][j] = el;
        }
    }
